...
import { parseLine, parseFile } from "./parser.js";
...
describe("parseLine", () => {
      ...
      message: {
        ...
        content: [
          { type: "toolCall", id: "c1", name: "edit",
            arguments: { path: "/home/doe/proj/src/foo.ts", edits: [
    ...
    parseLine({
      ...
      message: {
        ...
        content: [
          ...
          { type: "toolCall", id: "c2", name: "read", arguments: { path: "f" } },
  ...
  it("skips corrupt lines gracefully", () => {
    ...
    parseLine("not an object" as unknown as Record<string, unknown>, map);
    parseLine(null as unknown as Record<string, unknown>, map);
    parseLine(undefined as unknown as Record<string, unknown>, map);

    // entries without type should be skipped
    parseLine({ timestamp: "2026-06-08T00:00:00.000Z" }, map);
...
describe("parseFile", () => {
  ...
  it("parses a JSONL file into a day map", async () => {
    ...
    const lines = [
      JSON.stringify({ type: "session", version: 3, id: "s1", timestamp: "2026-06-08T10:00:00.000Z", cwd: "/home/doe/proj" }),
      ...
      "invalid json {broken",
      JSON.stringify({ type: "message", id: "m2", parentId: "m1", timestamp: "2026-06-08T10:02:00.000Z", message: { role: "assistant", content: [{ type: "text", text: "hey" }], model: "m", usage: { input: 100, output: 50, cacheRead: 0, cacheWrite: 0, totalTokens: 150, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0.01 } } } }),
    ];
